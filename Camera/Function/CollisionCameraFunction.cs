...
public class CollisionCameraFunction : CinemachineCameraFunction
{
    ...
    public override void OnDrawGizmosSelected()
    {
        ...
        Handles.DrawAAPolyLine(CameraStateData.OriginZoomDistance, 2, _endPosition, _lookAtPosition);
        ...
        for (int i = 0; i < _raycastPositions.Length; ++i)
        {
            ...
            Gizmos.DrawLine(_lookAtPosition, _raycastPositions[i]);
        }

        Gizmos.color = Color.green;
        Gizmos.DrawSphere(virtualCamera.transform.position, 0.1f);

        Gizmos.color = Color.gray;
        Gizmos.DrawSphere(_endPosition, COLLISION_PROBE_SIZE);

        Gizmos.color = Color.yellow;
        Gizmos.DrawSphere(_endPosition, 0.1f);

        Gizmos.color = Color.magenta;
        Gizmos.DrawSphere(MinHit.point, 0.1f);

        if (MinHit.collider == null)
        {
            OnScreenLog.Add(44, "Not Collide");
        }
        else
        {
            OnScreenLog.Add(44, "Collide :: " + MinHit.collider.name);
        }
